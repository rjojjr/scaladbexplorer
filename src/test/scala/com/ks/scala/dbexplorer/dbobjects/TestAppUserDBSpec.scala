...
class TestAppUserDBSpec extends WordSpecLike with Matchers {

  val time = System.currentTimeMillis()
  val users = Seq(
    AppUser(1, time, "test", "user1", "testuser1", "SomePW55#", false),
    AppUser(2, time, "test", "user2", "testuser2", "SomePW55#", false),
    AppUser(0, 0, null, null, null, null, false)
  )
  ...
  "TestDB" should {
    ...
    def test2(appUser: AppUser) = {
      create(appUser)
      val result =
        (findByUsernameAndPassword(appUser.username, appUser.password))
      delete(appUser)
      result should equal(appUser)
    }
    ...
    def test3(appUsers: Seq[AppUser]) = {
      val result =
        (findByUsernameAndPassword(appUsers(0).username, appUsers(0).password))
